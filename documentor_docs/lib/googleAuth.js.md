...
1. **Construct Redirect URL**: 
   - The function constructs a `redirectTo` URL using the current window's origin and appending `/auth/callback` with a query parameter `next` that encodes the `redirectPath`. This URL is used to redirect the user after authentication.

2. **Initiate OAuth Sign-In**:
   - The function calls `supabase.auth.signInWithOAuth` with the following parameters:
     - `provider`: Set to `"google"`, indicating that Google is the OAuth provider.
     - `options`: An object containing:
       - `redirectTo`: The URL to which the user will be redirected after authentication.
...
- **Redirect Path**: Ensure that the `redirectPath` provided is a valid path within your application to avoid redirect issues.
