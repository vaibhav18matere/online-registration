...
- **Purpose**: Computes the completion status of document uploads for a registration.
- **Parameters**:
  - `registration`: An object containing registration data.
- **Returns**: An object with the number of uploaded documents, total documents, and required missing documents.

This documentation provides a detailed overview of the `registrationService.js` file, explaining its purpose, key components, and the functionality of each function within the file.